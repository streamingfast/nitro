...
func ProduceBlock(
	...
	return ProduceBlockAdvanced(
		message.Header, txes, delayedMessagesRead, lastBlockHeader, statedb, chainContext, hooks, isMsgForPrefetch, runMode, tracer,
	)
}
...
func ProduceBlockAdvanced(
	...
	var hookedStateDB vm.StateDB = statedb

	if tracer != nil {
		hookedStateDB = state.NewHookedState(statedb, tracer)
	...
	for len(txes) > 0 || len(redeems) > 0 {
		...
		receipt, result, err := (func() (*types.Receipt, *core.ExecutionResult, error) {
			...
			evm := vm.NewEVM(blockContext, hookedStateDB, chainConfig, vm.Config{Tracer: tracer})
		...
		if tx.Type() == types.ArbitrumInternalTxType {
			...
			arbState, err = arbosState.OpenSystemArbosState(hookedStateDB, nil, true)
			if err != nil {
				return nil, nil, err
			}
	...
	FinalizeBlock(header, complete, hookedStateDB, chainConfig)
