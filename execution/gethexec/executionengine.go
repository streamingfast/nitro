...
func (s *ExecutionEngine) SetTracer(tracer *tracing.Hooks) {
	s.tracer = tracer
}
...
func (s *ExecutionEngine) sequenceDelayedMessageWithBlockMutex(message *arbostypes.L1IncomingMessage, delayedSeqNum uint64) (*types.Block, error) {
	...
	startTime := time.Now()
	block, statedb, receipts, err := s.createBlockFromNextMessage(&messageWithMeta, false, s.tracer)
	if err != nil {
		return nil, err
	}
	blockCalcTime := time.Since(startTime)
...
func (s *ExecutionEngine) createBlockFromNextMessage(msg *arbostypes.MessageWithMetadata, isMsgForPrefetch bool, tracer *tracing.Hooks) (*types.Block, *state.StateDB, types.Receipts, error) {
...
func (s *ExecutionEngine) digestMessageWithBlockMutex(num arbutil.MessageIndex, msg *arbostypes.MessageWithMetadata, msgForPrefetch *arbostypes.MessageWithMetadata) (*execution.MessageResult, error) {
	...
	block, statedb, receipts, err := s.createBlockFromNextMessage(msg, false, s.tracer)
	if err != nil {
		return nil, err
	}
